...
def render() -> None:
    ...
    with col2:
        hog_pca = st.slider("HOG PCA komponentów", 20, 200, 100, 10,
    ...
    # Sprawdź czy modele są zapisane
    ...
    if not st.button("🔬 Uruchom ewaluację 5-fold CV", type="primary", use_container_width=True):
        st.info("Kliknij aby uruchomić pełną ewaluację. Może potrwać kilka minut.")
        return
    ...
    all_results = {}  # clf_name → lista dicts per fold
    ...
    for clf_name in clf_names:
        ...
        for fold in folds:
            ...
            progress.progress(frac_start, text=f"{clf_name.upper()} — Fold {fold.fold_index + 1}/5: ekstrakcja cech...")
            ...
            # Wytrenuj na tym foldzie (nie ładuj z dysku — chcemy świeży model per fold)
            ...
            # Pre-compute top-k sortowania raz (O(n·c·log c) zamiast O(n²) w pętli)
        ...
        # Macierz pomyłek agregowana z wszystkich foldów
    ...
    for clf_name in clf_names:
        ...
        # Tabela per fold
        df = pd.DataFrame([{
            "Fold": r["fold"], "Accuracy": f"{r['acc']:.3f}",
            "Top-3": f"{r['top3']:.3f}", "Top-5": f"{r['top5']:.3f}",
            "Precision": f"{r['prec']:.3f}", "Recall": f"{r['rec']:.3f}",
            "F1 (macro)": f"{r['f1']:.3f}", "Czas [s]": f"{r['time_s']:.1f}",
        } for r in results])
        ...
        # Macierz pomyłek (zsumowana z 5 foldów)
    ...
    # Porównanie klasyfikatorów jeśli więcej niż jeden
    if len(clf_names) > 1:
    ...
    if st.button("📊 Eksportuj pełny eksperyment do Excela", use_container_width=True):
        ...
        st.caption(
            ...
            f"Summary per metoda, macierze pomyłek."
        )
